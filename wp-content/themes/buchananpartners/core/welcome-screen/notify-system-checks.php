...
if ( ! class_exists( 'Allegiant_Notify_System' ) ) {
	 ...
	 */
	class Allegiant_Notify_System extends Epsilon_Notify_System {
		/**
		 * @param $ver
		 *
		 * @return mixed
		...
		public static function widget_importer_exporter_title() {
			...
			if ( ! $installed ) {
				return __( 'Install: Widget Importer Exporter Plugin', 'buchananpartners' );
			}

			$active = self::check_plugin_is_active( 'widget-importer-exporter' );
			if ( $installed && ! $active ) {
				return __( 'Activate: Widget Importer Exporter Plugin', 'buchananpartners' );
			}
		...
		public static function wordpress_importer_title() {
			...
			if ( ! $installed ) {
				return __( 'Install: WordPress Importer', 'buchananpartners' );
			}

			$active = self::check_plugin_is_active( 'wordpress-importer' );
			if ( $installed && ! $active ) {
				return __( 'Activate: WordPress Importer', 'buchananpartners' );
			}
		...
		public static function wordpress_importer_description() {
			$installed = self::check_plugin_is_installed( 'wordpress-importer' );
			if ( ! $installed ) {
				return __( 'Please install the WordPress Importer to create the demo content.', 'buchananpartners' );
			}

			$active = self::check_plugin_is_active( 'wordpress-importer' );
			if ( $installed && ! $active ) {
				return __( 'Please activate the WordPress Importer to create the demo content.', 'buchananpartners' );
			}
		...
		public static function widget_importer_exporter_description() {
			$installed = self::check_plugin_is_installed( 'widget-importer-exporter' );
			if ( ! $installed ) {
				return __( 'Please install the WordPress widget importer to create the demo content', 'buchananpartners' );
			}

			$active = self::check_plugin_is_active( 'widget-importer-exporter' );
			if ( $installed && ! $active ) {
				return __( 'Please activate the WordPress Widget Importer to create the demo content.', 'buchananpartners' );
			}
